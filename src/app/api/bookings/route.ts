...
// ❌ total_price, prix_nuitee → JAMAIS acceptés depuis le client
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Advisory Lock PostgreSQL contre la race condition
    await db.query('BEGIN');
    await db.query('SELECT pg_advisory_xact_lock(hashtext($1))', [listing_id]);

    // Vérifier la disponibilité (dans la même transaction)
    ...
    // Prix calculé EXCLUSIVEMENT depuis la base de données
    const listing = await db.query(
      `SELECT prix_nuitee, title FROM listings
    ...
    const total_price          = prix_nuitee * nb_nuits;
    ...
    // Créer le booking en attente
    const booking = await db.query(
      `INSERT INTO bookings
         (listing_id, client_id, check_in, check_out,
          prix_nuitee_snapshot, total_price, commission_platform, montant_proprietaire, status)
    ...
    // Token de callback à usage unique (2h TTL)
    ...
    // Initier le paiement côté SERVEUR — l'ID marchand n'est jamais exposé
